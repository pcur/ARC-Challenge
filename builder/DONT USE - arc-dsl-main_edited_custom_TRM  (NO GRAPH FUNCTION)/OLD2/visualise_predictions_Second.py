...
def plot_task(sample, transformer, vae, device, save_dir=None):
    ...
    One figure per task showing:
        ...
        Row 3 : metrics panel (pixel acc, color acc, exact match, etc.)
    """
    ...
    # ── figure layout ─────────────────────────────────────────────────────
    # 4 rows: demo inputs | demo outputs | test analysis | metrics
    # Columns: max(n_train, 4) so the test row always has 4 slots
    n_cols  = max(n_train, 4)
    fig_w   = max(n_cols * 2.2, 10)

    # Row heights: demo rows are square grids, metrics row is taller text
    height_ratios = [2.2, 2.2, 2.2, 2.0]
    fig, axes = plt.subplots(
        4, n_cols,
        figsize=(fig_w, sum(height_ratios) + 0.6),
    ...
    # ── Row 3: metrics panel spanning all columns ─────────────────────────
    # Hide all individual axes in row 3 except the first, which we'll
    # use as a single wide panel by making the others invisible
    for col in range(1, n_cols):
        axes[3, col].set_visible(False)

    # Use the first axis but make it span visually via its position
    # We achieve a full-width panel by using a GridSpec subplot spanning
    # all columns — but since we already created the figure, we add a
    # new axes on top that spans the full width of row 3
    fig.canvas.draw()   # needed to get correct positions
    bbox0  = axes[3, 0].get_position()
    bboxN  = axes[3, n_cols-1].get_position()
    wide_ax = fig.add_axes([
        bbox0.x0,
        bbox0.y0,
        bboxN.x1 - bbox0.x0,
        bbox0.height,
    ...
    draw_metrics_panel(wide_ax, metrics_vae, metrics_tr, task_id)
