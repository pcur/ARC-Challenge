...
from OLD2.custom_object import grid_to_graph
from OLD2.gat_vae import GATVAE
from OLD2.graph_decoder import gat_vae_loss
...
def graph_to_pyg_data(graph: dict) -> Data:
    """
    Convert custom_object.py output into a PyTorch Geometric Data object.
...
def build_targets(graph: dict, max_nodes: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    ...
    custom_object.py node features:
      first 10 dims  = colors
      last 12 dims   = geometry

    custom_object.py edge features:
      [0]  dx
      [1]  dy
      [2]  manhattan
      [3]  dist
...
def make_batch(samples: List[Dict], max_nodes: int, device: str):
    ...
    for s in samples:
        ...
        target_color, target_node_geom, target_edge_cont, target_edge_bin = build_targets(graph, max_nodes)
    ...
    return batch, target_color, target_node_geom, target_edge_cont, target_edge_bin
...
def train_one_epoch(model, optimizer, samples, max_nodes, batch_size, device):
    ...
    for mini in chunk_list(samples, batch_size):
        batch, target_color, target_node_geom, target_edge_cont, target_edge_bin = make_batch(
...
def main():
    ...
    # Model matches full custom builder:
    #   node_in_dim = 22
    #   edge_in_dim = 12
    #   node_geom_dim = 12
    model = GATVAE(
