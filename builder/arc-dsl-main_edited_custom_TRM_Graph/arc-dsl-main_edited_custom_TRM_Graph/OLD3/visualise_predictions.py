...
def draw_metrics_panel(ax, metrics_vae, metrics_tr, task_id):
    ...
    # Layout constants — all in axes coordinates [0, 1]
    # Leave 10% top margin so header text never clips against the axis edge
    top        = 0.88   # header y position
    divider_y  = 0.78   # line below header
    data_top   = 0.72   # y of first data row
    data_range = 0.68   # total vertical space for data rows
    row_h      = data_range / n_rows

    # Column header row
    ax.text(0.02, top, "Metric",      color="white",   fontsize=8,
            fontweight="bold", va="center", transform=ax.transAxes)
    ax.text(0.45, top, "VAE Recon",   color="#4C9BE8", fontsize=8,
            fontweight="bold", va="center", transform=ax.transAxes)
    ax.text(0.75, top, "Transformer", color="#E8834C", fontsize=8,
            fontweight="bold", va="center", transform=ax.transAxes)
...
def plot_task(sample, transformer, vae, device, save_dir=None):
    ...
    #   row 1 — demo input grids
    #   row 2 — section label: "Demonstrations - Output"
    #   row 3 — demo output grids
    #   row 4 — section label: "Test Analysis"
    #   row 5 — test input | VAE recon | transformer pred | ground truth
    ...
    def add_row_label(row, text, color="#aaaaaa"):
        """Full-width section header label for a thin row."""
    ...
    # ── Row 2 label + Row 3: demo outputs ────────────────────────────────
    ...
    # ── Row 4 label + Row 5: test analysis ───────────────────────────────
    ...
    # ── Row 6: metrics panel — spans ALL columns ──────────────────────────
