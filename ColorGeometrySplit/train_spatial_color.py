...
CFG = dict(
    ...
    # Training
    epochs            = 1000,   # extended — model still improving at 500
    weight_decay      = 1e-3,
    lr_max            = 1e-4,
    lr_min            = 1e-6,
    lr_cycle_epochs   = 50,
...
def extract_geom_features(obj_batch, stats, device):
    ...
    Edge geom  : edge_attr cols [0:4, 7:10, 10:12]  normalised (E, 9)
                 ...
                 same_row, same_col, same_area,
                 area_ratio_ab, area_ratio_ba
    """
    x  = obj_batch.x.clone()
    ea = obj_batch.edge_attr.clone()
    ...
    x_geom    = x[:, 10:]                                              # (N, 12)
    edge_geom = torch.cat([ea[:, :4], ea[:, 7:10], ea[:, 10:]], dim=-1)  # (E, 9)

    # target spatial flags: same_row (7), same_col (8), same_area (9)
    ...
    with ctx:
        for obj_batch, _, _ in loader:
            ...
            loss, breakdown = spatial_geom_loss(
                ...
                edge_spatial_weight = cfg["edge_spatial_weight"],
            )
