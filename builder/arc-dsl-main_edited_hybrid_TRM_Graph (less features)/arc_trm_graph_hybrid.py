...
arc_trm_graph.py (this file):
    - Calls grid_to_graph() from hybrid_object2.py to extract objects
...
hand-engineered structural representation, combining:
    - Your object extraction work (hybrid_object2.py)
...
NODE_FEAT_DIM      = 110   # node: 10 color + 100 shape mask
EDGE_FEAT_DIM      = 5     # edge: dx, dy, touching, same_row, same_col
...
def build_sequence_tensors(
    ...
    # Pad all tokens to NODE_FEAT_DIM (110) so they can be stacked
    padded = []
    for t, tp in zip(tokens, types):
        if tp == 0:
            padded.append(t)                        # already 110-dim
        else:
            # Pad 5-dim edge features to 110-dim with zeros
...
def encode_grid_to_graph(raw_grid):
    """Convert raw grid → graph dict via hybrid_object2."""
...
def graph_to_targets(graph: dict, max_nodes: int, max_cells: int = None):
    """
    Build padded supervision targets from one output graph.
    Hybrid version — uses shape mask + bbox instead of cell coords.

    Returns:
        target_existence  : (max_nodes,)      float
        target_color      : (max_nodes,)      long
        target_node_shape : (max_nodes, 100)  float  flattened 10x10 mask
        target_bbox       : (max_nodes, 4)    float  normalised /30
    """
    nodes   = graph["nodes"]
    n_nodes = min(len(nodes), max_nodes)
    ...
    for ni in range(n_nodes):
        ...
        # Flatten 10x10 shape mask
        mask = node["shape_mask"]
        flat = []
        for r in range(10):
            for c in range(10):
                if r < len(mask) and c < (len(mask[r]) if r < len(mask) else 0):
                    flat.append(float(mask[r][c]))
                else:
                    flat.append(0.0)
        target_node_shape[ni] = torch.tensor(flat, dtype=torch.float32)

        # Bbox normalised by 30
        min_r, min_c, max_r, max_c = node["bbox"]
        target_bbox[ni] = torch.tensor(
            [min_r/30.0, min_c/30.0, max_r/30.0, max_c/30.0],
            dtype=torch.float32,
        )

    return (target_existence, target_color, target_node_shape, target_bbox)
...
def collate_batch(samples: List[Dict], device: str):
    ...
    for s in samples:
        ...
        tgts = graph_to_targets(s["test_output_graph"], MAX_NODES)
    ...
    # Stack targets — hybrid: existence, color, shape mask, bbox
    (tex, tcol, tshape, tbbox) = zip(*all_targets)
    targets = {
        "existence"   : torch.stack(tex).to(device),     # (B, N)
        "color"       : torch.stack(tcol).to(device),    # (B, N)
        "node_shape"  : torch.stack(tshape).to(device),  # (B, N, 100)
        "bbox"        : torch.stack(tbbox).to(device),   # (B, N, 4)
    }
...
class GraphTinyRecursiveModel(nn.Module):
    ...
    Input: interleaved node+edge feature sequence from hybrid_object2.py
    ...
    def __init__(
        ...
        # Hybrid heads: shape mask + bbox instead of cell coords
        self.node_shape_head = nn.Sequential(
            nn.Linear(hidden_size, hidden_size * 2), nn.SiLU(),
            nn.Linear(hidden_size * 2, max_nodes * 100),
        )
        self.node_bbox_head = nn.Sequential(
            nn.Linear(hidden_size, hidden_size), nn.SiLU(),
            nn.Linear(hidden_size, max_nodes * 4),
        )
    ...
    def _decode_from_hidden(self, z_h):
        ...
        shape_logits     = self.node_shape_head(h).view(B, N, 100)
        bbox_pred        = self.node_bbox_head(h).view(B, N, 4)

        return {
            "existence"  : existence_logits,
            "color"      : color_logits,
            "node_shape" : shape_logits,
            "bbox"       : bbox_pred,
        }

    def forward(self, seq_feats, seq_types, pad_mask, targets=None, n_sup=None):
        """
        seq_feats : (B, L, NODE_FEAT_DIM)
        seq_types : (B, L)
    ...
    def _compute_loss(self, preds, targets):
        """
        Hybrid loss — existence, color, shape mask, bbox.

        Losses:
            existence BCE  — all node slots
            color CE       — real nodes only
            shape mask MSE — real nodes only
            bbox MSE       — real nodes only
        """
        ...
        # Shape mask MSE (real nodes only)
        shape_flat     = preds["node_shape"].view(B*N, 100)
        shape_tgt_flat = targets["node_shape"].view(B*N, 100)
        if node_mask_flat.any():
            loss_shape = F.mse_loss(
                shape_flat[node_mask_flat],
                shape_tgt_flat[node_mask_flat],
            )
        else:
            loss_shape = preds["node_shape"].sum() * 0.0

        # Bbox MSE (real nodes only)
        bbox_flat     = preds["bbox"].view(B*N, 4)
        bbox_tgt_flat = targets["bbox"].view(B*N, 4)
        if node_mask_flat.any():
            loss_bbox = F.mse_loss(
                bbox_flat[node_mask_flat],
                bbox_tgt_flat[node_mask_flat],
            )
        else:
            loss_bbox = preds["bbox"].sum() * 0.0
...
def preds_to_grid(preds: dict, height: int, width: int, sample_idx: int = 0):
    ...
    Uses graph_to_grid_from_predictions from hybrid_object2.py.
    Hybrid path: shape mask + bbox (not cell coordinates).
    """
    existence_prob = torch.sigmoid(preds["existence"][sample_idx]).cpu().tolist()
    color_pred     = preds["color"][sample_idx].argmax(dim=-1).cpu().tolist()
    shape_pred     = preds["node_shape"][sample_idx].cpu().tolist()
    bbox_pred      = preds["bbox"][sample_idx].cpu().tolist()
...
def run_epoch(model, optimizer, samples, batch_size, device,
    ...
    with ctx:
        for chunk_idx, chunk in enumerate(chunks):
            ...
            loss, _ = model(seq_feats, seq_types, pad_mask, targets)
...
def main():
    ...
    save_path  = os.path.join(script_dir, "arc_trm_graph_hybrid_best.pt")
    ...
    print("Starting Graph TRM Hybrid training...\n")
