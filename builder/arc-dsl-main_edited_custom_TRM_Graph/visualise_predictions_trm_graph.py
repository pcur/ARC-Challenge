"""
visualise_predictions_trm_graph.py

Visualisation script for the Graph Tiny Recursive Model (arc_trm_graph.py).

Unlike visualise_predictions_trm.py which works on raw pixel tokens,
this script uses the graph representation from custom_object3.py —
nodes (objects) and edges (relationships) — as input to the TRM.

Inference pipeline:
    raw grids → grid_to_graph() → node/edge tokens → GraphTRM → preds_to_grid()
...
Usage:
    python visualise_predictions_trm_graph.py
"""
...
from arc_trm_graph import (
    ...
    MAX_NODES, MAX_CELLS_PER_NODE, NUM_COLORS,
    MAX_DEMOS, EMA_DECAY,
)
...
SEED         = 42
SAVE_FIGURES = True
OUTPUT_DIR   = "visualisations_trm_graph"
...
def draw_diff(ax, pred, target, title="Diff Map"):
    """
    Green = correct foreground pixel
    Dark grey = correct background pixel
    Red = wrong pixel
    """
    ...
    for r in range(h_t):
        for c in range(w_t):
            ...
            p = pred[r][c]
            if t == p and t == bg:
                diff[r, c] = [0.1, 0.1, 0.1]     # dark grey — bg correct
            elif t == p:
                diff[r, c] = [0.18, 0.80, 0.25]  # green — fg correct
            else:
...
def trm_graph_predict(
    ...
    """
    model.eval()
    seq_feats, seq_types, pad_mask, _, test_sizes, _ = collate_batch([sample], device)
...
# ─────────────────────────────────────────────────────────────────────────────
# DATASET BUILDER (one sample per task, no augmentation)
# ─────────────────────────────────────────────────────────────────────────────

def build_vis_samples(train_path: str, max_files: int = None) -> List[Dict]:
    """
    Build one sample per task for visualisation — no augmentation.
    Adds raw grids to each sample so we can display them.
    """
...
def draw_metrics_panel(ax, metrics, task_id):
    ...
    ax.text(0.55, top, "Graph TRM",    color="#2ECC40", fontsize=8,
            fontweight="bold", va="center", transform=ax.transAxes)
...
def plot_task(sample: Dict, model: GraphTinyRecursiveModel,
    ...
    # Inference
    pred    = trm_graph_predict(sample, model, device)
    metrics = compute_metrics(pred, test_gt)
    ...
    add_row_label(4, "  TEST  —  Input  |  Graph TRM Prediction  |  Ground Truth  |  Diff Map")

    panels = [
        lambda ax: draw_grid(ax, test_in_raw, title="Test Input"),
        lambda ax: (draw_grid(ax, pred,
                              title=f"Graph TRM  {'✓' if exact else '✗'}",
                              title_color="#2ECC40" if exact else "#FF4136")
                    if pred is not None else draw_blank(ax, "Prediction\nfailed")),
        lambda ax: draw_grid(ax, test_gt, title="Ground Truth"),
        lambda ax: draw_diff(ax, pred, test_gt, title="Diff Map"),
    ]
    ...
    # Row 6: metrics panel
    draw_metrics_panel(fig.add_subplot(gs[6, :]), metrics, task_id)

    verdict = "✓ EXACT MATCH" if exact else "✗ NO MATCH"
    fig.suptitle(
        f"Task: {task_id}  [Graph TRM]\n"
...
def plot_summary(results: List[Dict], save_dir=None):
    ...
    ax = axes[0]
    bars = ax.bar(["Exact Match", "Mean Pixel Acc"], [acc, mean_px],
                  color=["#2ECC40", "#4C9BE8"], width=0.5, edgecolor="white")
    ax.set_ylim(0, 100)
    ax.set_ylabel("Accuracy (%)", fontsize=11)
    ax.set_title(f"Graph TRM — Results over {n} tasks",
    ...
    if save_dir:
        path = os.path.join(save_dir, "_summary_trm_graph.png")
    ...
    print(f"\n{'='*50}")
    print(f"GRAPH TRM RESULTS SUMMARY  ({n} tasks evaluated)")
    print(f"{'='*50}")
...
def main():
    ...
    # ── load Graph TRM checkpoint ─────────────────────────────────────────
    print("Loading Graph TRM checkpoint...")
    ...
    model.eval()
    print(f"  Graph TRM loaded  (epoch {ckpt.get('epoch','?')}, "
          f"best val loss: {ckpt.get('best_val_loss','N/A')})\n")

    # ── build samples ─────────────────────────────────────────────────────
    print("Building task samples...")
    all_samples = build_vis_samples(TRAIN_PATH)
